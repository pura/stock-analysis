#!/usr/bin/env python3
"""
Scrape Yahoo Finance Top Gainers (first page: 25 rows) and store into SQLite
...
def main():
    ...
    html = http_get(YAHOO_GAINERS_URL)
    ...
    try:
        init_db(conn)
        scraped_at = insert_rows(conn, rows)
    finally:
        conn.close()

    print(f"Saved {len(rows)} rows into {db_path} table '{TABLE_NAME}' at {scraped_at} UTC")
